...
import { FiGithub, FiLinkedin, FiMail, FiArrowRight, FiSmartphone } from "react-icons/fi";
import { SiApple, SiAndroid } from "react-icons/si";
import { motion } from "framer-motion";

const socialLinks = [
  { href: "https://github.com/kristalpithwa", icon: FiGithub, label: "GitHub", hover: "hover:text-white", target: "_blank" },
  { href: "https://www.linkedin.com/in/kristal-pithwa", icon: FiLinkedin, label: "LinkedIn", hover: "hover:text-blue-400", target: "_blank" },
  { href: "mailto:crystalpithwa@gmail.com", icon: FiMail, label: "Email", hover: "hover:text-purple-400" },
];
        ...
        <div className="w-[200px] h-[420px] bg-gradient-to-b from-[#1a1a2e] to-[#16162a] rounded-[40px] p-[5px] shadow-[0_30px_80px_-20px_rgba(59,130,246,0.2),0_0_40px_rgba(139,92,246,0.08)] border border-white/[0.06] relative">
          ...
          <div className="w-full h-full bg-[#0a0e1a] rounded-[35px] overflow-hidden relative">
            <div className="flex justify-between items-center px-5 pt-7 pb-1.5">
              <span className="text-[7px] text-white/40 font-geist font-semibold">9:41</span>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }} className="px-4 pt-2 pb-3">
              <div className="text-[8px] text-white/30 font-geist">Good morning</div>
              <div className="text-[11px] text-white/80 font-geist font-semibold">Dashboard</div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.4 }} className="flex gap-2 px-3 mb-3">
            ...
            {[1.6, 1.8, 2.0].map((delay, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay }}
                ...
                <div className={`w-7 h-7 rounded-lg bg-gradient-to-br ${
                  ["from-teal-400/20 to-teal-600/10", "from-amber-400/20 to-amber-600/10", "from-rose-400/20 to-rose-600/10"][i]
                }`} />
                <div className="flex-1">
                  <div className={`h-[4px] bg-white/12 rounded mb-1 ${["w-14", "w-12", "w-10"][i]}`} />
                  <div className={`h-[3px] bg-white/5 rounded ${["w-20", "w-16", "w-18"][i]}`} />
                </div>
                <div className="w-6 h-2.5 bg-white/[0.04] rounded-full" />
              </motion.div>
            ))}

            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 2.4, type: "spring", stiffness: 260, damping: 20 }}
            ...
            <div className="absolute bottom-0 left-0 right-0 h-11 bg-[#0a0e1a]/80 backdrop-blur border-t border-white/[0.03] flex items-center justify-around px-6">
              {["bg-blue-400/30", "bg-white/8", "bg-white/8", "bg-white/8"].map((c, i) => (
                <div key={i} className={`w-4 h-4 ${i === 0 ? "rounded-lg" : i === 3 ? "rounded-full" : "rounded-md"} ${c}`} />
              ))}
            </div>
...
const Hero = () => {
  return (
    <section className="min-h-[85vh] flex flex-col justify-center relative py-14">
      ...
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[350px] rounded-full pointer-events-none"
        style={{ background: "radial-gradient(ellipse at center, rgba(56,189,248,0.06) 0%, rgba(139,92,246,0.04) 40%, transparent 70%)" }} />

      <motion.div animate={{ y: [0, -16, 0], opacity: [0.2, 0.7, 0.2] }} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
        className="absolute top-[18%] right-[12%] w-1.5 h-1.5 bg-teal-400 rounded-full pointer-events-none hidden lg:block" />
      <motion.div animate={{ y: [0, 18, 0], opacity: [0.15, 0.5, 0.15] }} transition={{ duration: 7, repeat: Infinity, ease: "easeInOut", delay: 1 }}
          ...
          <div className="flex-1 text-center lg:text-left">
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
              ...
              <div className="inline-flex items-center gap-2 glass-card rounded-full px-3.5 py-1.5">
                ...
                <span className="text-[11px] text-gray-400 tracking-wide font-geist">Open to work</span>
              </div>
            </motion.div>

            <motion.h1 className="font-geist tracking-tighter leading-[0.92] mb-4"
            ...
            <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7, duration: 0.5 }}
              ...
              <span className="inline-flex items-center gap-1">Hey!
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img alt="" src="/images/waving.png" className="w-4 h-4 animate-wave" />
              </span>{" "}
              I build native {"&"} cross-platform mobile apps for iOS {"&"} Android.
              Specialized in React Native with a focus on smooth animations and
              pixel-perfect UI.
            </motion.p>
